	...
	*/
	class product
	{
		...
		public function insert_product($date,$files){
			...
			$product_spec = mysqli_real_escape_string($this->db->link, $date['product_spec']);
			$import_price= mysqli_real_escape_string($this->db->link, $date['import_price']);
			...
			if($productName == "" || $productQuantity == "" || $category == "" || $brand == "" || $product_desc == "" || $product_spec == ""||$import_price=="" || $price == "" || $type == "" || $file_name == ""){
				$alert = "<span class='error'>Các trường không được để trống</span>";
				return $alert;
			}else{
				move_uploaded_file($file_temp, $uploaded_image);

				$query = "INSERT INTO tbl_product(productName,product_remain,productQuantity,catId,brandId,product_desc,product_spec,import_price,price,type,image) VALUES('$productName','$productQuantity','$productQuantity','$category','$brand','$product_desc','$product_spec','$import_price','$price','$type','$unique_image')";
			...
			if($sliderName=="" || $type==""){
				...
				if(!empty($file_name)){
					...
					if ($file_size > 2048000) {
		    		 ...
		    		 $alert = "<span class='success'>Ảnh phải nhỏ hơn 2MB!</span>";
					return $alert;
				    } 
					...
					$result = $this->db->insert($query);
					if($result){
						$alert = "<span class='success'>Thêm slider thành công</span>";
						return $alert;
					}else {
						$alert = "<span class='error'>Thêm slider không thành công</span>";
						return $alert;
					}
				}
		...
		public function del_slider($id)
		{
			...
			if($result){
				$alert = "<span class='success'>Xóa slider thành công</span>";
				return $alert;
			}else {
				$alert = "<span class='success'>Xóa slider không thành công</span>";
				return $alert;
			}
		}
		public function update_quantity_product($data,$files,$id){
			...
			if($product_more_quantity == ""){
				...
				return $alert; 
					...
					// $qty = $product_more_quantity + $productQuantity;
		...
		public function update_product($data,$files,$id){
			...
			$product_spec = mysqli_real_escape_string($this->db->link, $data['product_spec']);
			$import_price=mysqli_real_escape_string($this->db->link, $data['import_price']);
			...
			if($productName=="" || $brand=="" || $category=="" || $product_desc=="" || $product_spec==""||$import_price=="" ||$price=="" || $type==""){
				$alert = "<span class='error'>Các trường không được để trống</span>";
				return $alert; 
			}else{
				if(!empty($file_name)){
					...
					{
				     // echo "<span class='error'>You can upload only:-".implode(', ', $permited)."</span>";	
				    $alert = "<span class='success'>Bạn chỉ có thể tải lên:-".implode(', ', $permited)."</span>";
					return $alert;
					}
					move_uploaded_file($file_temp,$uploaded_image);
					$query = "UPDATE tbl_product SET
				...
				$result = $this->db->update($query);
					if($result){
						$alert = "<span class='success'>Sản phẩm cập nhật thành công</span>";
						return $alert;
					}else{
						$alert = "<span class='error'>Sản phẩm cập nhật thành công không thành công</span>";
						return $alert;
					}
		...
		public function del_product($id)
		{
			...
			if($result){
				...
				$alert = "<span class='success'>Xóa sản phẩm không thành công</span>";
				return $alert;
			}
		}
		...
		public function insertWishlist($productid, $customer_id)
		{
			...
			if($result_check_wlist){
				$msg = "<span class='error'>Sản phẩm đã được thêm vào danh sách yêu thích</span>";
				return $msg;
			}else{
			...
			if($insert_wlist){
						$alert = "<span class='success'>Thêm sản phẩm vào danh sách yêu thích thành công</span>";
						return $alert;
					}else{
						$alert = "<span class='error'>Thêm sản phẩm vào danh sách yêu thích thất bại</span>";
						return $alert;
					}
			}
		}
